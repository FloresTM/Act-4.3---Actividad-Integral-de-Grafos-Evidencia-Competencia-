...
template <class Vertex>
void bfs(const Vertex& start, const UnweightedGraph<Vertex>* graph, int &count, int k) {
	std::queue<Vertex> pending;
	std::map<Vertex, bool> visited;
	int graph_nodes;
	std::vector<Vertex> ans;
	typename std::set<Vertex>::iterator itr;
	typename std::vector<Vertex>::iterator itr_vec;
	int depth = 0;

	graph_nodes = graph->getVertexes().size() - 1;

	pending.push(start);
	while (!pending.empty()) {
		depth += 1;
		int pending_size = pending.size();
		...
		for(int i = 0; i < pending_size; i++){
			...
			for (itr = connected.begin(); itr != connected.end(); itr++) {
				if((visited.count(*itr) == 0)){	
					pending.push(*itr);
					if(!(std::find(ans.begin(),ans.end(),*itr) != ans.end()) && depth <= k){
						ans.push_back(*itr);
					}
				}
			}
		}
		count = graph_nodes - ans.size();
	}
}
